...
const InteractiveQuery: React.FC<InteractiveQueryProps> = ({
  ...
  const getLandCoverColor = (landCover: string): string => {
    switch (landCover) {
      case 'Buildings': return '#8B4513';
      case 'Concrete': return '#708090';
      case 'Dense Vegetation': return '#228B22';
      case 'Sparse Vegetation': return '#9ACD32';
      case 'Unvegetated Asphalt': return '#2F4F4F';
      case 'Vegetated Asphalt': return '#696969';
      case 'Water Body': return '#4682B4';
  ...
  const getTemperatureBackground = (temp: number): string => {
    ...
    if (temp >= 39) return 'bg-orange-50 border-orange-200';
      ...
      <div className="bg-white max-w-md w-full mx-4 shadow-2xl border border-slate-200 overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-orange-600 p-4">
          ...
          <div className="text-xs text-blue-100 mt-1">
            LST Analysis | {selectedDate} {acquisitionTimes[selectedDate as keyof typeof acquisitionTimes]}
          </div>
        </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className={`p-4 border ${getTemperatureBackground(queryData.temperature)}`}>
            ...
            <div className="text-center">
              <div className={`text-3xl font-bold mb-1 ${getTemperatureColor(queryData.temperature)}`}>
                {queryData.temperature}°C
              </div>
              <div className="text-xs text-slate-600">Landsat 8 LST (30m resolution)</div>
            </div>
          </div>
          ...
          <div className="bg-slate-50 p-4 border border-slate-200">
            ...
            <div className="text-xs text-slate-600 mt-2">
              OpenStreetMap-derived classification
            </div>
          </div>

          {/* MUHI Status */}
          <div className={`p-4 border ${
            queryData.muhiStatus.some(status => status.includes('≥40°C'))
              ? 'bg-red-50 border-red-200'
              : queryData.muhiStatus.some(status => status.includes('≥39°C'))
              ...
              {queryData.muhiStatus.map((status, index) => (
                <div key={index} className="flex items-center gap-2">
                  <div className={`w-2 h-2 rounded-full ${
                    status.includes('≥40°C') ? 'bg-red-600' :
                    status.includes('≥39°C') ? 'bg-orange-500' :
                    'bg-green-600'
                  }`}></div>
                  <span className={`text-sm ${
                    status.includes('≥40°C') ? 'text-red-700 font-medium' :
                    status.includes('≥39°C') ? 'text-orange-700 font-medium' :
                    'text-green-700'
                  }`}>
                    {status}
                  </span>
          ...
          <div className="bg-blue-50 p-4 border border-blue-200">
            ...
            <div className="text-xs text-blue-700 space-y-1">
              ...
              <div>• NDVI → Emissivity → LST calculation</div>
              <div>• Acquisition: {selectedDate} at {acquisitionTimes[selectedDate as keyof typeof acquisitionTimes]}</div>
              <div>• Spatial Resolution: 30 meters</div>
          ...
          <div className="bg-slate-50 p-4 border border-slate-200">
            ...
            <div className="space-y-1 text-xs text-slate-600">
              <div className="flex items-center gap-2">
                ...
                <span>Canopy Threshold: ≥40°C (Dense Vegetation Reference)</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 bg-orange-500 rounded-full"></div>
                <span>Top 2% Threshold: ≥39°C (98th Percentile)</span>
              </div>
            </div>
          </div>
